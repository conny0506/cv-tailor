...
export default async function ResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isManual = !gen.analysis;

  return (
    <div className="space-y-6">
      <section>
        <h1 className="text-2xl font-bold mb-1">
          {isManual ? "Manuel Proje Seçimi" : gen.analysis!.role}
        </h1>
        <p className="text-sm text-slate-600">
          {isManual ? (
            <>
              {gen.projects.length} proje · CV dili: {gen.language.toUpperCase()} · şablon:{" "}
              {gen.template}
            </>
          ) : (
            <>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded p-4 space-y-3">
          <h2 className="font-semibold">
            {isManual ? "Seçilen Projeler" : "Seçilen Projeler"} ({gen.projects.length})
          </h2>
